...
if __name__ == "__main__":
    ...
    checkpoint = torch.load('/home/shirshak/BPEye_Project_2024/logs/train/runs/4) glaucoma_resnet_airogs_FOCAL/checkpoints/epoch_018.ckpt')
    ...
    test_data_paths = sorted(glob('/mnt/Enterprise2/shirshak/Glaucoma_Dataset_Drishti-GS/Drishti-GS1_preprocessed_cropped_separated_train_test_val/test/*/*.png'))
    ...
    precision = precision_score(labels_list, predicted_list, average='weighted', zero_division=0.0)
    recall = recall_score(labels_list, predicted_list, average='weighted', zero_division=0.0)
    f1 = f1_score(labels_list, predicted_list, average='weighted', zero_division=0.0)
    # roc_auc_score_ = roc_auc_score(labels_list.cpu().numpy(), predicted_list.cpu().numpy(), average='weighted')

    confusion_matrix_chart = confusion_matrix(labels_list, predicted_list)
    cm_display = ConfusionMatrixDisplay(confusion_matrix = confusion_matrix_chart, display_labels = ['NRG', 'RG'])
    
    cm_display.plot()
    
    plt.savefig('/home/shirshak/BPEye_Project_2024/zzz_tests/CM_dristi.png', dpi=500)
    plt.close()
    ...
    for count, wrong_case in enumerate(wrong_cases[:5]):
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig(f"/home/shirshak/BPEye_Project_2024/zzz_tests/wrong_cases_glaucoma_dristi/wrong{count}.jpg")
        plt.close()

    for count, right_case in enumerate(right_cases[:5]):
        fig, ax = plt.subplots(nrows=1,ncols=2,figsize=(8,5))
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig(f"/home/shirshak/BPEye_Project_2024/zzz_tests/right_cases_glaucoma_dristi/right{count}.jpg")
        plt.close()
